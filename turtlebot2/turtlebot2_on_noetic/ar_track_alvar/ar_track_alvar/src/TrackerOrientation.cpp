...
void TrackerOrientation::Project(CvMat* state, CvMat* projection, void* param)
{
  ...
  cvProjectPoints2(tracker->_object_model, &rot_mat, &zero_tra,
                   &(tracker->_camera->calib_K), &(tracker->_camera->calib_D),
                   projection);
...
bool TrackerOrientation::UpdatePose(IplImage* image)
{
  ...
  double foo = opt->Optimize(par, &image_observations_sub, 0.0005, 5, Project,
                             this, alvar::Optimization::TUKEY_LM);
...
bool TrackerOrientation::UpdateRotationOnly(IplImage* gray, IplImage* image)
{
  ...
  while (it != _F_v.end())
  {
    ...
    if (f->status3D == Feature::NONE)
    {
      ...
      // inv(K)*[u v 1]'*scale
      wx = object_scale * (fpu.x - _camera->calib_K_data[0][2]) /
           _camera->calib_K_data[0][0];
      wy = object_scale * (fpu.y - _camera->calib_K_data[1][2]) /
           _camera->calib_K_data[1][1];
      wz = object_scale;
    ...
    {
      if (f->status3D == Feature::NONE)
        cvCircle(image, cvPoint(int(f->point.x), int(f->point.y)), 3,
                 CV_RGB(255, 0, 0), 1);
        ...
        cvCircle(image, cvPoint(int(f->point.x), int(f->point.y)), 2,
                 CV_RGB(255, 0, 255), 1);
    }
        ...
        f->status3D == Feature::IS_INITIAL)
    {
      ...
      if (image)
        cvLine(image, cvPoint(int(p2d[0]), int(p2d[1])),
               cvPoint(int(f->point.x), int(f->point.y)), CV_RGB(255, 0, 255));

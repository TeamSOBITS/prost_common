...
bool SimpleSfM::AddMultiMarker(const char* fname,
...
{
  marker_detector.MarkerToEC(container, marker_id, edge_length, pose, 0, 0,
                             1023);
}

float PointVectorFromCamera(CvPoint3D32f p3d, CvPoint3D32f& p3d_vec,
                            Pose* camera_pose)
{
  ...
  return std::sqrt(p3d_vec.x * p3d_vec.x + p3d_vec.y * p3d_vec.y +
                   p3d_vec.z * p3d_vec.z);
}

void CreateShadowPoint(CvPoint3D32f& p3d_sh, CvPoint3D32f p3d, CameraEC* cam,
                       Pose* camera_pose, float parallax_length,
                       float triangulate_angle)
{
  ...
  float shadow_point_dist =
      float(pl / std::abs(tan(triangulate_angle * 3.1415926535 /
                              180.)));  // How far we need the shadow point to
                                        // have the parallax limit match
...
bool SimpleSfM::Update(IplImage* image, bool assume_plane, bool triangulate,
                       float reproj_err_limit, float triangulate_angle)
{
  ...
  // tf.Track(image, 0, container, 1, 1024, 65535); // Track with adding
  // features
  ...
  // #4. Add previously triangulated features to container if they are
  // visible...
  if (remember_3d_points)
  {
    ...
    while (iter != container_triangulated.end())
    {
      if (container.find(iter->first) == container.end())
      {
        ...
        {
          CvScalar s = cvGet2D(mask, int(iter->second.projected_p2d.y),
  ...
  if (assume_plane)
  {
    for (iter = container.begin(); iter != iter_end; iter++)
    {
      ...
      if (!f.has_p3d && f.estimation_type < 1)
      {
        ...
        // if (l > plane_dist_limit) cam.Get3dOnDepth(&pose, f.p2d,
        // plane_dist_limit, f.p3d);
        cam.Get3dOnDepth(&pose, f.p2d, plane_dist_limit, f.p3d);

        // TODO: Now we don't create a shadow point for plane points. This is
        // because
        //       we don't want to remember them as 3d points in
        //       container_triangulated. We probably get the same benefit just
        //       by assuming that everything is on plane?
  ...
  if (triangulate)
  {
    for (iter = container.begin(); iter != iter_end; iter++)
    {
      ...
      if (!f.has_p3d && !f.has_stored_pose)
      {
        ...
        CreateShadowPoint(f.p3d_sh, f.p3d, &cam, &pose, parallax_length,
                          triangulate_angle);
      }
      if (!f.has_p3d && f.has_stored_pose && f.estimation_type < 2)
      {
        double dist_sh_sq = (f.projected_p2d_sh.x - f.projected_p2d.x) *
                            (f.projected_p2d_sh.x - f.projected_p2d.x);
        dist_sh_sq += (f.projected_p2d_sh.y - f.projected_p2d.y) *
                      (f.projected_p2d_sh.y - f.projected_p2d.y);
  ...
  while (iter != container.end())
  {
    ...
    if (iter->second.type_id > 0)
    {
      ...
      else
      {
        ...
        dist_sq +=
            (f.p2d.y - f.projected_p2d.y) * (f.p2d.y - f.projected_p2d.y);
  ...
  // #8. Remember good features that have little reprojection error while the
  // parallax is noticeable
  if (remember_3d_points && (container_triangulated.size() < max_triangulate))
  {
    ...
    {
      if ((iter->second.type_id > 0) &&
          (container_triangulated.find(iter->first) ==
           container_triangulated.end()))
      {
        Feature& f = iter->second;
        double dist_sq =
            (f.p2d.x - f.projected_p2d.x) * (f.p2d.x - f.projected_p2d.x);
...
bool SimpleSfM::UpdateTriangulateOnly(IplImage* image)
{
  ...
  {
    if (pose_ok && (iter->second.type_id == 1) &&
        (!iter->second.has_stored_pose) && (!iter->second.has_p3d))
    {
...
bool SimpleSfM::UpdateRotationsOnly(IplImage* image)
{
  ...
  else  // if(pose_ok) // Tracking going on...
  {
    ...
    if (pose_ok)
    {
      ...
      CvMat rot = cvMat(4, 4, CV_64F, rot_mat);  // Rotation matrix (difference)
                                                 // from the tracker
      CvMat mod = cvMat(4, 4, CV_64F,
                        gl_mat);  // Original modelview matrix (camera location)
  ...
  if (pose_ok)
  {
    if (n_markers < 1)
    {
      ...
      for (; iter != iter_end; iter++)
      {
        if ((iter->second.type_id == 1) && (!iter->second.has_p3d) &&
            (iter->second.has_p2d))
        {
          CvPoint2D32f _p2d = iter->second.p2d;
          cam.Get3dOnDepth(&pose_difference, _p2d, 1, iter->second.p3d);
    ...
    double err_features =
        cam.Reproject(container, &pose_difference, 1, false, false, 100);
  ...
  for (; iter != iter_end; iter++)
  {
    ...
    if (f.has_p2d)
    {
      ...
      if (pose_ok)
      {
        ...
        {
          cvLine(rgba, cvPointFrom32f(f.projected_p2d),
                 cvPointFrom32f(f.projected_p2d_sh), CV_RGB(0, 255, 0));
        }
        // Reprojection error
        if (f.has_p3d)
        {

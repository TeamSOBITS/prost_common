...
static void ProjectRot(CvMat* state, CvMat* projection, void* x)
{
  ...
  cvProjectPoints2(object_model, &rot_mat, &zero_tra, &(camera->calib_K),
                   &(camera->calib_D), projection);
...
static void Project(CvMat* state, CvMat* projection, void* x)
{
  ...
  cvReshape(projection, projection, 2, 1);
  cvProjectPoints2(object_model, &rot_mat, &tra_mat, &(camera->calib_K),
                   &(camera->calib_D), projection);
  cvReshape(projection, projection, 1, count);
}

bool CameraEC::UpdatePose(const CvMat* object_points, CvMat* image_points,
                          Pose* pose, CvMat* weights)
{
  double rot[3];
...
bool CameraEC::UpdatePose(const CvMat* object_points, CvMat* image_points,
                          CvMat* rot, CvMat* tra, CvMat* weights)
{
  ...
  double tmp = opt->Optimize(par, image_points, 0.0005, 2, Project, &pparams,
                             alvar::Optimization::TUKEY_LM, 0, 0, weights);
...
bool CameraEC::UpdateRotation(const CvMat* object_points, CvMat* image_points,
                              Pose* pose)
{
  double rot[3];
  CvMat rotm = cvMat(3, 1, CV_64F, rot);
                              ...
                              CvMat* rot, CvMat* tra)
{
  ...
  double tmp = opt->Optimize(par, image_points, 0.0005, 2, ProjectRot, &pparams,
...
void GetPointOnLine(const Pose* pose, Camera* camera, const CvPoint2D32f* u,
                    CvMat* P)
{
...
bool MidPointAlgorithm(CvMat* o1, CvMat* o2, CvMat* p1, CvMat* p2,
                       CvPoint3D32f& X, double limit)
{
...
// todo
bool CameraEC::ReconstructFeature(const Pose* pose1, const Pose* pose2,
                                  const CvPoint2D32f* u1,
                                  const CvPoint2D32f* u2, CvPoint3D32f* p3d,
                                  double limit)
{
...
void CameraEC::Get3dOnPlane(const Pose* pose, CvPoint2D32f p2d,
                            CvPoint3D32f& p3d)
{
...
void CameraEC::Get3dOnDepth(const Pose* pose, CvPoint2D32f p2d, float depth,
                            CvPoint3D32f& p3d)
{
  double wx, wy, wz;
  Camera::Undistort(p2d);

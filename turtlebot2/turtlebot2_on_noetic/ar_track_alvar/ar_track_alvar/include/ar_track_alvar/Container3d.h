...
/** \brief Functor class for \e Container3d \e Sort() to sort the search base
 * using distance to specified origin. */
template <class T>
...
public:
  Container3dSortDist(Container3d<T>& _container, const CvPoint3D32f _orig)
    : container(_container), orig(_orig)
  {
  }
  bool operator()(size_t i1, size_t i2)
  {
    float x1 = container[i1].first.x - orig.x,
          x2 = container[i2].first.x - orig.x;
    float y1 = container[i1].first.y - orig.y,
          y2 = container[i2].first.y - orig.y;
    float z1 = container[i1].first.z - orig.z,
          z2 = container[i2].first.z - orig.z;
...
/** \brief Functor class for \e Container3d \e Limit() to limit the search space
 * with distance */
template <class T>
...
public:
  Container3dLimitDist(Container3d<T>& _container, const CvPoint3D32f _orig,
                       float _dist_limit)
 ...
 * ...)
 *
 * You can store any information in 3D using this container. Each element in the
 * container has an unique id that it can be referenced with using \e
 * operator[](). The indices are from 0 to \e size(). You can find specific
 * index using \e GetIndex(Iterator &iter) or \e GetIndex(T *p) .
 *
 * In addition the Container3d contains also a 'search space' that can be
 * iterated through using \e begin() and \e end(). This 'search space' can be
 * limited using \e Limit() , sorted using \e Sort() and reseted using \e
 * ResetSearchSpace(). You specify what to limit/sort using specified functors.
 * In ALVAR there exists functors \e Container3dLimitDist , \e
 * Container3dSortSize and \e Container3dSortDist . But you can quite well make
 * your own versions (see example below).
 *
 * The implementation is optimized for a situation where there are a lot of
 * searches between every time the search space is limited/ordered. This is the
 * reason we use vector containers internally. The idea is that later we will
 * improve the class by providing more ways for limiting and sorting the search
 * space; for example Frustum culling.
 *
 * Usage:
 *
 * \code
...
{
public:
  /** \brief \e node_type for storing data. 3D-position is paired with the data
   * content. */
...
protected:
  ...
  /** \brief Possibly limited set of indices for \e data in somehow "optimal"
   * search order */
  std::vector<size_t> search_space;

public:
  /** \brief Add \e _data in the container and associate it with 3D position \e
   * _pos */
  ...
  /** \brief Iterator for going through the items in \e Container3d in the
   * specified order
   *
   * The idea is that the content in \e Container3d can be sorted and limited in
   * different ways. After sorting/limiting the content the \e iterator (\e
   * Begin() and \e End() ) can be used for accessing the data items in optimal
   * order.
   */
  ...
  /** \brief Provides an iterator pointing to the beginning of the
   * limited/sorted 3D content */
  Iterator begin()
  {
    return Iterator(this, search_space.begin());
  }

  /** \brief Provides an iterator pointing to the end of the limited/sorted 3D
   * content */
  Iterator end()
  {
  ...
  /** \brief Get absolute reference usable with \e operator[]() based on the
   * iterator */
  ...
  /** \brief Instead of \e Iterator we can use also absolute references for data
   * with \e operator[]() */
